module.exports.config = {
    ...
    hasPermssion: 2,
    credits: "vanh",
    description: "Treo tag theo yêu cầu của bạn( bản cực vip)",
...
module.exports.run = async function({ api, args, Users, event }) {
  ...
  // Check syntax
  if (Object.keys(mentions) == 0 && args[0] != "stop") return reply("Vui lòng tag người bạn muốn treo tag");
  if ((!solantag || !time) && args[0] != "stop") reply("Sai cú pháp !!!\n\nHD: treotag @tag Hi 1 5");
  if (isNaN(solantag) && args[0] != "stop") return reply("Số lần tag phải là một con số\n\nHD treotag @tag Hi 10 2");
  ...
  reply(`===>Bắt đầu treo theo ý chủ nhân<===`);
