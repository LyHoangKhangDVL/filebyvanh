  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`ꪜ${name} gỡ cái dmmmm\n=====Lời em nó lỡ sủaa=====\n${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`🔰${name} vừa bị bố mày soi được ${getMsg.attachment.length} tệp đính kèm.${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}` : ""}`,
              attachment:[],
