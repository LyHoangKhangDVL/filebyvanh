...
module.exports.run = async function({ api, event, args, getText, Users, Currencies }) {
	...
	if (global.configModule[this.config.name].maxPlayers < 10 || global.configModule[this.config.name].maxPlayers > 10) global.configModule[this.config.name].maxPlayers = 10;
	//well, happy new year!
	var moneyBet = 0;
	switch (args[0]) {
		case 'create':
