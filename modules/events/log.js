module.exports.config = {
  ...
  version: "1.0.0",
  credits: "vanh",//*được sự chỉ dẫn nhiệt tình của dc-nam
...
module.exports.run = async function ({ api, event, Users, Threads, Currencies }) {
  ...
  switch (event.logMessageType) {
    case "log:thread-name": {
        ...
        task = "「{name}」đã thay đổi tên nhóm " + newName + "";
    ...
    case "log:unsubscribe": {
      if (event.logMessageData.leftParticipantFbId == botID) {
        ...
        data.banned = true;
        var reason = "「{name}」 đã từ bỏ mọi điều xa hoa bot đem lại để học cách cảm nhận nỗi đau xa rìa bot";
        data.reason = reason || null;
        data.dateAdded = time;
        await Threads.setData(event.threadID, { data });
        global.data.threadBanned.set(event.threadID, { reason: data.reason, dateAdded: data.dateAdded });

        task = "「{name}」 đã kick bot ra khỏi nhóm!"
      }
      break;
    }
    default:
